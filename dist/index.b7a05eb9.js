...
const genreInput = document.getElementById("input2");
const fLimit = limitInput?.value ?? 10;
...
function searchData() {
    ...
    for(let k = 0; k < rows.length; k++){
        ...
        if (rows[k].innerText.includes(genreInput?.value.toUpperCase())) ;
        else myTable?.removeChild(rows[k]);
    }
}
function fetchsomeSearchedNewData(fetchLimit) {
    // Use the user input to control the number of random users to fetch
    const genreval = genreInput?.value.toUpperCase();
...
function fetchOneData(fetchLimit) {
    // Use the user input to control the number of random users to fetch
    const genreval = genreInput?.value.toUpperCase();
    let val = Number(fetchLimit);
    _axiosDefault.default.request({
...
function fetchsomeNewData(fetchLimit) {
    // Use the user input to control the number of random users to fetch
    const genreval = genreInput?.value.toUpperCase();
    let val = Number(fetchLimit);
    let k = 0;
